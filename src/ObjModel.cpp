...
void ObjModel::PrintObjModelInfo() {
...
void ModelPart::ApplyModelMatrix(glm::mat4x4 model_matrix) {
    glUniformMatrix4fv(g_model_uniform, 1 , GL_FALSE , glm::value_ptr(model_matrix));
    glUniform1i(g_object_id_uniform, id);
}

void ModelPart::setTextures(GLuint* diffuse, GLuint* specular, GLuint* normals) {
    if (diffuse != NULL) {
